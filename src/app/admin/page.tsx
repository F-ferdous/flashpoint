...
import { DollarSign, ShieldCheck, Users, Wallet } from "lucide-react";
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/70">Overview of today’s activity and platform stats.</p>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <CardStat icon={<Users className="h-4 w-4" />} label="Active Agents" value="128" delta="+4.1%" tone={{ bg: "bg-emerald-500/15", text: "text-emerald-600 dark:text-emerald-300" }} />
        <CardStat icon={<Wallet className="h-4 w-4" />} label="Wallet Balance" value="$42,310" delta="+2.3%" tone={{ bg: "bg-rose-500/15", text: "text-rose-600 dark:text-rose-300" }} />
        <CardStat icon={<DollarSign className="h-4 w-4" />} label="Transfers Today" value="$8,920" delta="+12.0%" tone={{ bg: "bg-sky-500/15", text: "text-sky-600 dark:text-sky-300" }} />
        <CardStat icon={<ShieldCheck className="h-4 w-4" />} label="Verifications" value="56" delta="-3.4%" tone={{ bg: "bg-amber-500/15", text: "text-amber-700 dark:text-amber-300" }} />
      </section>
