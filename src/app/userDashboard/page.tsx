...
export default function UserHomePage() {
    ...
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        onSnapshot(doc(db, "Customers", user.uid), (snap) => {
          ...
          const cid =
            String(x.CustomerID || x.customerId || x.CustomerId || "").trim() ||
            null;
          setCustomerId(cid);
        });
      }
  ...
  const total = useMemo(
    () => bonusFromAgent + earnedOnline + bonusFromAdmin,
    [bonusFromAgent, earnedOnline, bonusFromAdmin]
  );
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">
            {t("dash.user.header_title")}
          </h1>
          <p className="hidden sm:block text-sm text-foreground/70">
            Track your earnings and complete tasks to earn rewards
          </p>
          {customerId && (
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <SummaryCard
          label="Bonus from Agent"
          value={`${bonusFromAgent} pts`}
          delta={"—"}
        />
        <SummaryCard
          label="Earned Online"
          value={`${earnedOnline} pts`}
          delta={"—"}
        />
        <SummaryCard
          label="Bonus from Admin"
          value={`${bonusFromAdmin} pts`}
          delta={"—"}
        />
      </section>
      ...
      <section className="rounded-xl border border-black/10 dark:border-white/10 bg-[var(--surface)]/60 dark:bg-white/5">
        <div className="p-4 flex items-center justify-between">
          ...
          <Link href="/userDashboard/earnings">
            <Button
              variant="ghost"
              className="text-sm hover:text-[var(--brand)]"
            >
              View All Earnings
            </Button>
          </Link>
        ...
        <ul className="divide-y divide-black/10 dark:divide-white/10">
          {recentEarnings.length > 0 ? (
            recentEarnings.map((earning) => {
              ...
              const source =
                earning.source === "adsterra"
                  ? "Offer"
                  : earning.source?.includes("chargeback")
                  ? "Offer Refund"
              ...
              return (
                <li
                  key={earning.id}
                  className="p-4 text-sm grid gap-1 sm:grid-cols-[180px_1fr]"
                >
                  <div className="text-foreground/70">
                    {earning.ts?.toDate
                      ? earning.ts.toDate().toLocaleDateString()
                      : "Recent"}
                  </div>
...
function CardStat({
  ...
  value,
  delta,
  tone = { bg: "bg-[var(--brand-15)]", text: "text-foreground" },
}: {
    ...
    <div className="rounded-xl border border-black/10 dark:border-white/10 bg-[var(--surface-2)] p-4">
      <div className="flex items-center justify-between text-sm text-foreground/70">
        <span className="inline-flex items-center gap-2">
          <span
            className={`grid place-items-center rounded-md p-1.5 ${tone.bg} ${tone.text}`}
          >
            {icon}
          </span>
          {label}
