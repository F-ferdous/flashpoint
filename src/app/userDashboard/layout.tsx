...
const navItems = [
  {
    href: "/userDashboard/earnings",
    k: "dash.user.nav.earnings",
    icon: Wallet,
    tone: {
  ...
  {
    href: "/userDashboard/referals",
    k: "dash.user.nav.referals",
    icon: Gift,
    tone: {
      bg: "bg-violet-500/15",
          ...
          <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-xs sm:text-sm">
              ...
              {((customer as any)?.CustomerID ||
                (customer as any)?.customerId) && (
                <Badge variant="outline" className="truncate max-w-[9rem]">
                  ID:{" "}
                  {(customer as any)?.CustomerID ||
                    (customer as any)?.customerId}
                </Badge>
        ...
        <div className="mx-auto max-w-7xl grid md:grid-cols-[260px_1fr] gap-6 px-4 md:px-6 py-6">
          ...
          <aside className="hidden md:flex sticky top-20 self-start rounded-xl border border-black/10 dark:border-white/10 bg-[var(--surface-2)] p-4 h-[calc(100dvh-96px)] flex-col">
            ...
            {((customer as any)?.CustomerID ||
              (customer as any)?.customerId) && (
              <div
                className="mt-2 px-2 text-xs text-foreground/70 truncate"
                title={
