    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/70">Welcome back. Here’s your account overview.</p>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card label="Wallet Balance" value="$ 820" />
        <Card label="Active Subscriptions" value="2" />
        <Card label="Points" value="4,250" />
      </section>
        ...
        <ul className="divide-y divide-black/10 dark:divide-white/10">
          {Array.from({ length: 6 }).map((_, i) => (
            <li key={i} className="p-4 text-sm grid gap-1 sm:grid-cols-[180px_1fr]">
              ...
              <div>Transaction #{(i + 1) * 7} • {i % 2 ? "Debit" : "Credit"} • $ {(20 + i * 5).toFixed(2)}</div>
            </li>
          ))}
        </ul>
      </section>
