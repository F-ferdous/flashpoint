...
export async function POST(req: NextRequest) {
  try {
    ...
    const counterDocRef = adminDb.collection("counters").doc(`agent_${distId}`);
    const agentId = await adminDb.runTransaction(async (tx) => {
      const snap = await tx.get(counterDocRef);
      const current = (snap.exists ? (snap.data() as any)?.seq : 0) || 0;
      const next = current + 1;
      tx.set(counterDocRef, { seq: next, type: "agent", districtId: distId, updatedAt: Date.now() }, { merge: true });
